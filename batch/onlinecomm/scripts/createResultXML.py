...
def main(path):#, runs):
    for i in xrange(1, 13):
        ...
        poblacionNotConverged = "Population" + str(i) + "-NotConvergedNormativeSystems.dat"
        nameNotCon = 'NotConvergence'
        fileNotCon = path + poblacionNotConverged

        # if(os.path.isfile(file) == False):
        poblacionConverged = "Population" + str(i) + "-ConvergedNormativeSystems.dat"
        fileCon = path + poblacionConverged
        nameCon = 'Convergence'

        previous = ''
        ns = 1
        if(os.path.isfile(fileNotCon) == True):
            with open(fileNotCon) as f:
                convergence = etree.Element(nameNotCon)
                for line in f:
                    line = line.rstrip('\n')
                    if(previous == 'A'):
                        count.text = line
        ...
        previous = ''
        ns = 1
        if(os.path.isfile(fileCon) == True):
            with open(fileCon) as f:
                convergence = etree.Element(nameCon)
                for line in f:
                    line = line.rstrip('\n')
                    if(previous == 'A'):
                        count.text = line
